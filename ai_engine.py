...
SAMBANOVA_MODEL = "Meta-Llama-3.1-8B-Instruct"  # 8B = faster, less rate limit
...
async def _call_sambanova(messages: list, mode: str) -> str | None:
    """Call SambaNova — skip immediately if rate limited, no retries."""
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            if resp.status_code == 429:
                logger.warning("SambaNova rate limited — skipping to Gemini")
                return None
...
async def get_ai_response(
    ...
    """
    1️⃣ SambaNova 8B → 2️⃣ Gemini 2.0 Flash → Static message
    No retries — instant fallback on rate limit!
    """
    ...
    # ❌ Both failed
    logger.error("Both AI engines failed!")
